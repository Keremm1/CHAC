...
class ExperienceBuffer():
    ...
    def add_curious(self, curious_value):    
        self.curious_buffer.append(curious_value)
        self.curious_size += 1

        if self.curious_size >= self.max_buffer_size:
            beg_index = int(np.floor(self.max_buffer_size/6))
            self.curious_buffer = self.curious_buffer[beg_index:]
            self.size -= beg_index
